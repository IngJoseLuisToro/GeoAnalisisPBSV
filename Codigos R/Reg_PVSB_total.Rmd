...
getwd()
sueldodf <- read.csv("Total_estudio_PBSV_regionmetropol_v2.csv")
...
```{r}
modelo <- lm(PBSV ~ EDAD + SEXO + FPS + PFP +
               YHL + YOI + YKL + AFPER2018, data = sueldodf )
summary(modelo)


round(cor(x = sueldodf, method = "pearson"), 3)

modelo2 <- lm(PBSV ~ YOI, data = sueldodf )
summary(modelo2)

modelo3 <- lm(PBSV ~ FPS, data = sueldodf )
summary(modelo3)
```
